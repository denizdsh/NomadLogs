import { Link } from "react-router";
import { ROUTES } from "~/constants/routes";
import { OAuthButton } from "~/components/ui/OAuthButton";
import { Logo } from "~/components/ui/Logo";

export function meta() {
  return [
    { title: "Login — NomadLogs" },
    {
      name: "description",
      content: "Sign in to NomadLogs with your favorite OAuth provider.",
    },
  ];
}

function GoogleIcon() {
  return (
    <svg viewBox="0 0 24 24" width="20" height="20" aria-hidden="true">
      <path
        fill="#4285F4"
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"
      />
      <path
        fill="#34A853"
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
      />
      <path
        fill="#FBBC05"
        d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
      />
      <path
        fill="#EA4335"
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
      />
    </svg>
  );
}

function AppleIcon() {
  return (
    <svg
      viewBox="0 0 24 24"
      width="20"
      height="20"
      fill="currentColor"
      aria-hidden="true"
    >
      <path d="M17.05 20.28c-.98.95-2.05.88-3.08.4-1.09-.5-2.08-.48-3.24 0-1.44.62-2.2.44-3.06-.4C2.79 15.25 3.51 7.59 9.05 7.31c1.35.07 2.29.74 3.08.8 1.18-.24 2.31-.93 3.57-.84 1.51.12 2.65.72 3.4 1.8-3.12 1.87-2.38 5.98.48 7.13-.57 1.5-1.31 2.99-2.54 4.09zM12.03 7.25c-.15-2.23 1.66-4.07 3.74-4.25.29 2.58-2.34 4.5-3.74 4.25z" />
    </svg>
  );
}

function GitHubIcon() {
  return (
    <svg
      viewBox="0 0 24 24"
      width="20"
      height="20"
      fill="currentColor"
      aria-hidden="true"
    >
      <path d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0 1 12 6.844a9.59 9.59 0 0 1 2.504.337c1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.02 10.02 0 0 0 22 12.017C22 6.484 17.522 2 12 2z" />
    </svg>
  );
}

function FacebookIcon() {
  return (
    <svg
      viewBox="0 0 24 24"
      width="20"
      height="20"
      fill="currentColor"
      aria-hidden="true"
    >
      <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
    </svg>
  );
}

import { useState, useEffect } from "react";
import { useSearchParams } from "react-router";
import { useAuth } from "~/providers/AuthProvider";
import { useToast } from "~/providers/ToastProvider";
import { API_URL } from "~/config";

export default function Login() {
  const { user } = useAuth();
  const { toast } = useToast();
  const [searchParams] = useSearchParams();
  const [loadingProvider, setLoadingProvider] = useState<string | null>(null);

  // Show error messages from OAuth redirect failures
  useEffect(() => {
    const error = searchParams.get("error");
    if (error) {
      toast(decodeURIComponent(error), "error");
    }
  }, [searchParams]);

  /**
   * Initiates the OAuth flow by fetching the consent URL from the backend
   * and redirecting the user's browser to the provider's login page.
   */
  const handleOAuthLogin = async (provider: string) => {
    const providerLower = provider.toLowerCase();

    // Only Google is supported with real OAuth for now
    if (providerLower !== "google") {
      toast(`${provider} sign-in is not available yet.`, "info");
      return;
    }

    setLoadingProvider(providerLower);

    try {
      const response = await fetch(`${API_URL}/api/auth/${providerLower}/url`);
      const data = await response.json();

      if (!response.ok || !data.url) {
        throw new Error(data.error || "Failed to get authentication URL.");
      }

      // Redirect the browser to the OAuth consent screen
      window.location.href = data.url;
    } catch (err: any) {
      console.error("OAuth initiation error:", err);
      toast(err.message || "Failed to start authentication.", "error");
      setLoadingProvider(null);
    }
  };

  return (
    <article className="flex items-center justify-center min-h-[calc(100vh-140px)] px-6 py-12 animate-fade-in">
      <section className="w-full max-w-md space-y-8">
        {/* Logo + Welcome */}
        <header className="text-center space-y-4 flex flex-col items-center">
          <Link to={ROUTES.HOME} aria-label="NomadLogs Home">
            <Logo size="lg" />
          </Link>
          <section>
            <h1 className="text-headline-lg text-on-surface">
              Welcome back, explorer
            </h1>
            <p className="text-body-md text-on-surface-muted mt-1">
              Sign in to document your journeys and connect with fellow
              travellers
            </p>
          </section>
        </header>

        {/* OAuth Buttons */}
        <section className="space-y-3">
          <OAuthButton
            provider="google"
            onClick={() => handleOAuthLogin("GOOGLE")}
            icon={<GoogleIcon />}
            loading={loadingProvider === "google"}
          />
          <OAuthButton
            provider="apple"
            onClick={() => handleOAuthLogin("APPLE")}
            icon={<AppleIcon />}
            disabled
          />
          <OAuthButton
            provider="github"
            onClick={() => handleOAuthLogin("GITHUB")}
            icon={<GitHubIcon />}
            disabled
          />
          <OAuthButton
            provider="facebook"
            onClick={() => handleOAuthLogin("FACEBOOK")}
            icon={<FacebookIcon />}
            disabled
          />
        </section>

        {/* Terms notice */}
        <footer className="text-center">
          <p className="text-label-md text-on-surface-muted">
            By continuing, you agree to our{" "}
            <Link to={ROUTES.TERMS} className="text-primary hover:underline">
              Terms of Service
            </Link>{" "}
            and{" "}
            <Link to={ROUTES.PRIVACY} className="text-primary hover:underline">
              Privacy Policy
            </Link>
          </p>
        </footer>
      </section>
    </article>
  );
}
